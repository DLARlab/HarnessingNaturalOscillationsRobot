...
int main(int argc, char **argv)
{
    ...
    if(def_frame == coord::WORLD)
    {
        ...
        const double radius = 1.5*0;    //m
        tf::Quaternion q;
        while(ros::ok())
        {
            model_state_pub.pose.position.x = radius * sin(2*0*M_PI*(double)time_ms/period);
            model_state_pub.pose.position.y = radius * cos(2*0*M_PI*(double)time_ms/period);
            model_state_pub.pose.orientation = tf::createQuaternionMsgFromRollPitchYaw(0, 0, 0);//createQuaternionMsgFromRollPitchYaw(0, 0, - 2*M_PI*(double)time_ms/period);
