...
void State_SwingTest::_torqueCtrl(){
    ...
    Vec3 _hPosFrontHip =  _ctrlComp->robotModel->getFootPosition(*_lowState, 0, FrameType::HIP);
    Vec3 _bPosFrontHip =  _ctrlComp->robotModel->getFootPosition(*_lowState, 0, FrameType::BODY);

    std::cout << "_hPosFrontHip: " << _hPosFrontHip.transpose() << std::endl;
    std::cout << "_bPosFrontHip: " << _bPosFrontHip.transpose() << std::endl;
