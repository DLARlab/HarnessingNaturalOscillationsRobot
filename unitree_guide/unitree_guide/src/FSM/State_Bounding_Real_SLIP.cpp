...
void State_Bounding_Real_SLIP::getUserCmd(){
    ...
    if ( counterflight < 20){
          _vbCmdBodyDhc(0) = counterflight * 0.05 + 0.05;
          _vCmdUser(0) = _vbCmdBodyDhc(0);
        }
    else if ( counterflight <= 100){
        _vbCmdBodyDhc(0) = (counterflight - 20) * 0.05 + 20 * 0.05 + 0.05;
        _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
    else if ( counterflight > 100){
      _vbCmdBodyDhc(0) = (counterflight - 100) * 0.05 + 80 * 0.05 + 20 * 0.05 + 0.05;
      _vCmdUser(0) = _vbCmdBodyDhc(0);
    }
...
void State_Bounding_Real_SLIP::calcCmd(){
    /* Movement */
        ...
        // }   
        //  _pitchCmd = 0.0; 
        _pitchCmd = (-0.05 - 0.02 * _vCmdUser(0))+ (0.05 + 0.02 * _vCmdUser(0)) * sin ((2 * M_PI/strideIteration) * counterUse + (1-0.3 * _vCmdUser(0)) * M_PI/2);
