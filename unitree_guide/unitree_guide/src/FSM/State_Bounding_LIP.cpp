...
State_Bounding_LIP::State_Bounding_LIP(CtrlComponents *ctrlComp)
             :FSMState(ctrlComp, FSMStateName::BOUNDING_LIP, "bounding_lip"), 
...
State_Bounding_LIP::~State_Bounding_LIP(){
    delete _gait;
}

void State_Bounding_LIP::enter(){
...
FSMStateName State_Bounding_LIP::checkChange(){
...
bool State_Bounding_LIP::checkStepOrNot(){
...
// void State_Bounding_LIP::setHighCmd(double vx, double vy, double wz){
//     _vbCmdBody(0) = vx;
//     _vbCmdBody(1) = vy;
//     _vbCmdBody(2) = 0; 
//     _dYawCmd = wz;
...
void State_Bounding_LIP::getUserCmd(){
    /* Movement */
...
void State_Bounding_LIP::calcCmd(){
    /* Movement */
        ...
        // if (_vbCmdBody(0) < 3){
        //     _pitchCmd = (0.03 - 0.01 * _vbCmdBody(0) ) + (0.1 + 0.02 * _vbCmdBody(0))/2 * cos((2 * M_PI/strideIteration) * counterUse - M_PI/(15 - 3 *  _vbCmdBody(0))); // calculate the desired pitch angle
        // }
        // else if (_vbCmdBody(0) < 4){
        //      _pitchCmd = -0.02 * ( _vbCmdBody(0) - 3) + (0.16 - 0.04 *(_vbCmdBody(0) - 3) )/2 * cos((2 * M_PI/strideIteration) * counterUse- M_PI/(6 - 2 *  (_vbCmdBody(0) - 3))); // calculate the desired pitch angle
        // }
        // else{
        ...
        if (_vbCmdBody(0) < 2){
            _pitchCmd = (- 0.05 * _vbCmdBody(0) ) + (0.05 + 0.015 * _vbCmdBody(0)) * cos((2 * M_PI/strideIteration) * counterUse - M_PI/(15 - 6.5 *  _vbCmdBody(0))); // calculate the desired pitch angle
        }
        else if (_vbCmdBody(0) < 4){
             _pitchCmd = -0.08 - 0.01 * ( _vbCmdBody(0) - 2) + (0.05 + 0.02 *(_vbCmdBody(0) - 1) ) * cos((2 * M_PI/strideIteration) * counterUse- M_PI/(2 - 0.2 *  (_vbCmdBody(0) - 2))); // calculate the desired pitch angle
        }
        else{
            _pitchCmd = _lowState->getPitch();
        }    
    ...
    if((*_contact)(0) == 1 || (*_contact)(1) == 1){
        ...
        counterFront += 1.0;
        // _pitchCmd = 0.15 * sin ((2 * M_PI/strideIteration) * counterUse + M_PI/2); // calculate the desired pitch angle    
    ...
    if(file_posTorso8.is_open()){file_posTorso7<< _footForce[0]<< "\n";}
 ...
 */
void State_Bounding_LIP::calcTau(){
    // if((*_contact)(0) == 1 || (*_contact)(1) == 1){
...
////////////////////////////////////////////////////////
    ...
    if (counter > 2000){
        if((*_contact)(0) == 1 || (*_contact)(1) == 1){
            ...
            if((*_phase)(1) > 0.4){
                for (int i = 0; i < 2; i++){
                    _forceFeetGlobalAdd[i] =  0 * (_wPosFhc(2) - _wfhcd(2)) + 0 * (_VelFhc(2) - 0);
                }
            ...
            if((*_phase)(2) > 0.4){
                for (int i = 2; i < 4; i++){
                    _forceFeetGlobalAdd[i] =  1000 * (_wPosRhc(2) - _wrhcd(2)) + 1 * (_VelRhc(2) - 0);
                }
                _Kpp = Vec3(150, 70, 250).asDiagonal();
                _Kdp = Vec3(20, 20, 30).asDiagonal();
                _kpw = Vec3(400, 880, 400).asDiagonal(); 
        ...
        if((*_contact)(0) == 1 || (*_contact)(1) == 1){
            ...
            if((*_phase)(2) > 0.4){
                for (int i = 2; i < 4; i++){
                    _forceFeetGlobalAdd[i] =  101 * (_wPosRhc(2) - _wrhcd(2)) + 0.1 * (_VelRhc(2) - 0);
 ...
 * 
 */
void State_Bounding_LIP::calcQQd(){
...
Vec3 State_Bounding_LIP::xProd(Vec3 vec1, Vec3 vec2){
    Vec3 vec3;
...
void State_Bounding_LIP::plotData(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Bounding_LIP::plotSubPlot(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data, int legNum){
...
void State_Bounding_LIP::plotGRF(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
...
void State_Bounding_LIP::plotXZ(std::vector<std::vector<double>>& desd_data, std::vector<std::vector<double>>& actual_data){
